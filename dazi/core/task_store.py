...
KEY CONCEPTS:
  ...
  2. High water mark file (.hwm) prevents ID reuse after deletion
...
class TaskStore:
    ...
    Storage layout:
        ...
        ├── .hwm           — high water mark (max ID ever used)
        ├── 1.json         — task files
        ├── 2.json
        └── ...
    """
    ...
    def __init__(self, tasks_base_dir: Path, list_id: str = DEFAULT_LIST_ID) -> None:
        ...
        self._hwm_path = self._list_dir / HIGH_WATER_MARK_FILE
    ...
    def _next_id(self) -> int:
        """Get the next task ID by incrementing the high water mark."""
        self._list_dir.mkdir(parents=True, exist_ok=True)

        if self._hwm_path.exists():
            current = int(self._hwm_path.read_text(encoding="utf-8").strip())
        else:
            current = 0

        next_id = current + 1
        self._hwm_path.write_text(str(next_id), encoding="utf-8")
        return next_id
    ...
    def reset(self) -> None:
        """Remove all tasks and reset the high water mark. For testing only."""
        if self._list_dir.exists():
            for path in self._list_dir.glob("*.json"):
                path.unlink()
            if self._hwm_path.exists():
                self._hwm_path.unlink()
...
def task_create(subject: str, description: str, activeForm: str = "", metadata: dict[str, object] | None = None) -> str:
    ...
    from dazi.core._singletons import task_store
